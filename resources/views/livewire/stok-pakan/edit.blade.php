    ...
    public function update(): void
    {
        ...
        DB::transaction(function () use ($barang) {
            ...
            if ($this->kotor > 0) {
                ...
                $trxTelur1 = Transaksi::where('invoice', 'like', "INV-$tanggal-PKN1-$suffix")->first();
                if ($trxTelur1) {
                    $totalKotor = ($barang->hpp ?? 0) * ($this->kotor ?? 0);
                    $trxTelur1->update([
                        'type' => 'Kredit',
            ...
            $trxTelur2 = Transaksi::where('invoice', 'like', "INV-$tanggal-PKN2-$suffix")->first();
            if ($trxTelur2) {
                $totalPecah = ($barang->hpp ?? 0) * ($this->pecah ?? 0);
                $trxTelur2->update([
                    'total' => $totalPecah,
...
<div class="p-4 space-y-6">
    <x-header title="Edit Transaksi Stok Pakan" separator progress-indicator />
