...
new class extends Component {
    ...
    {
        return [
            ['key' => 'tanggal', 'label' => 'Tanggal', 'class' => 'w-32'],
            ['key' => 'nama_barang', 'label' => 'Nama Barang', 'class' => 'w-56'],
            ['key' => 'total_jumlah', 'label' => 'Jumlah (kg)', 'class' => 'w-48'],
    ...
    public function pembelianTelur(): LengthAwarePaginator
    {
        return DetailTransaksi::query()
            ->select(
                ...
                DB::raw('barang.name as nama_barang'),
                DB::raw('SUM(detail_transaksis.kuantitas) as total_jumlah'),
                DB::raw('SUM(detail_transaksis.kuantitas * detail_transaksis.value) as total_harga')
            )
            ...
            ->when($this->filterType, fn($q) => $q->where('transaksi.type', $this->filterType)) // ✅ filter Debit/Kredit
...
<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-10 gap-4 items-end mb-4">
        ...
        <div class="md:col-span-2">
            <x-select label="Tipe Transaksi" placeholder="Semua"
                :options="$types"
