...
new class extends Component {
    ...
    public function updateStatus(): void
    {
        DB::transaction(function () {
            ...
            $detailBelis = DetailTransaksi::where('transaksi_id', $transaksiBeli->id)->orderBy('id')->get();
            ...
            */
                ...
                $invoiceHutang = 'INV-' . $tanggal . '-UTG-' . $str;

                /*
            |--------------------------------------------------------------------------
            | Cari Kategori Hutang
            |--------------------------------------------------------------------------
            */

                $client = Client::findOrFail($transaksi->client_id);
                ...
                $kategori = Kategori::where('name', $kategoriName)->firstOrFail();
            ...
            */
                ...
                foreach ($detailRetur as $index => $retur) {
                    ...
                    $stokBatch->update([
                        'qty_masuk' => $detailBelis[$index]->kuantitas - $retur->kuantitas,
                        'qty_sisa' => $detailBelis[$index]->kuantitas - $retur->kuantitas,
                    ]);
                }
...
<div class="p-4 space-y-6">
    <x-header title="Transaksi Retur Pakan" separator progress-indicator>
        <x-slot:actions>
            <div class="flex flex-row sm:flex-row gap-2">
                ...
                <x-button label="Create" link="/sentrat-kembali/create" responsive icon="o-plus" class="btn-primary" />
            </div>
        </x-slot:actions>
    </x-header>
    ...
    <x-card class="overflow-x-auto">
        <x-table :headers="$headers" :rows="$transaksi" :sort-by="$sortBy" with-pagination
            link="sentrat-kembali/{id}/show?invoice={invoice}">
                ...
                <div class="flex">
                    ...
                    @if (Auth::user()->role_id == 1 ||
                        ...
                        <x-button icon="o-pencil"
                            link="/sentrat-kembali/{{ $transaksi->id }}/edit?invoice={{ $transaksi->invoice }}"
