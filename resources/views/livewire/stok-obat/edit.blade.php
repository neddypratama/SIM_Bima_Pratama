    ...
    public function update(): void
    {
        ...
        DB::transaction(function () use ($barang) {
            ...
            if ($this->kotor > 0) {
                ...
                $trxKotor = Transaksi::where('invoice', 'like', "INV-$tanggal-RTN-$suffix")->first();
                if ($trxKotor) {
                ...
                $trxTelur1 = Transaksi::where('invoice', 'like', "INV-$tanggal-OBT1-$suffix")->first();
                if ($trxTelur1) {
                    $totalKotor = ($barang->hpp ?? 0) * ($this->kotor ?? 0);
                    $trxTelur1->update([
                        'type' => 'Kredit',
            ...
            $trxPecah = Transaksi::where('invoice', 'like', "INV-$tanggal-KDL-$suffix")->first();
            if ($trxPecah) {
            ...
            $trxTelur2 = Transaksi::where('invoice', 'like', "INV-$tanggal-OBT2-$suffix")->first();
            if ($trxTelur2) {
                $totalPecah = ($barang->hpp ?? 0) * ($this->pecah ?? 0);
                $trxTelur2->update([
                    'total' => $totalPecah,
            ...
            <div class="lg:grid grid-cols-8 gap-4">
                ...
                <div class="col-span-6 grid gap-3">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end p-3 rounded-xl">
                        <x-input label="Obat Bertambah" wire:model.lazy="tambah" type="number" step="0.01" min="0" />
                        <x-input label="Obat Berkurang" wire:model.lazy="kurang" type="number" step="0.01" min="0" />
                        <x-input label="Obat Return" wire:model.lazy="kotor" type="number" step="0.01"  />
                        <x-input label="Obat Kadaluarsa" wire:model.lazy="pecah" type="number" step="0.01" min="0" />
                    </div>
                </div>
            </div>
        </x-card>
