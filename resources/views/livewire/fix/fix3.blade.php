...
new class extends Component {
    ...
    public function isValidTotal($trx): bool
    {
        $detailTotal = $trx->details->sum('sub_total');
        return round($trx->total, 2) == round($detailTotal, 2);
    }

    public function syncTotal($id): void
    {
        ...
        $trx->update([
            'total' => round($detailTotal, 2),
        ]);

        $this->success("Total {$trx->invoice} disinkronkan", position: 'toast-top');
    }
    ...
    public function syncAll(): void
    {
        ...
        foreach ($transaksis as $trx) {
            ...
            if (round($trx->total, 2) != round($detailTotal, 2)) {
                $trx->update([
                    'total' => round($detailTotal, 2),
                ]);
                $updated++;
        ...
        $this->success("Berhasil sinkron {$updated} transaksi", position: 'toast-top');
    }
    ...
    {
        return [['key' => 'invoice', 'label' => 'Invoice', 'class' => 'w-24'], ['key' => 'name', 'label' => 'Rincian', 'class' => 'w-48'], ['key' => 'tanggal', 'label' => 'Tanggal', 'class' => 'w-16'], ['key' => 'client.name', 'label' => 'Client', 'class' => 'w-16'], ['key' => 'client.keterangan', 'label' => 'Tipe Client', 'class' => 'w-16'], ['key' => 'total', 'label' => 'Total', 'class' => 'w-24', 'format' => ['currency', 0, 'Rp']], ['key' => 'type', 'label' => 'Tipe', 'class' => 'w-16'], ['key' => 'status_check', 'label' => 'Status', 'class' => 'w-16']];
    }
    ...
    public function transaksis(): LengthAwarePaginator
    {
        return Transaksi::query()
            ...
            ->when($this->validStatus, function ($q) {
                if ($this->validStatus === 'valid') {
                    $q->whereRaw('(SELECT SUM(sub_total) FROM detail_transaksis WHERE transaksi_id = transaksis.id) = total');
                } else {
                    $q->whereRaw('(SELECT SUM(sub_total) FROM detail_transaksis WHERE transaksi_id = transaksis.id) != total');
                }
            })
...
<div>
    ...
    <x-header title="Daftar Transaksi" separator progress-indicator>
        <x-slot:actions>
            <div class="flex flex-row sm:flex-row gap-2">
                ...
                <x-button wire:click="syncAll" icon="o-arrow-path" class="btn-warning" spinner>
                    Sinkron Semua
                </x-button>
    ...
    <x-card>
        <x-table :headers="$headers" :rows="$transaksis" :sort-by="$sortBy" with-pagination
            ...
            {{-- STATUS --}}
            @scope('cell_status_check', $trx)
                @php
                    $detailTotal = $trx->details->sum('sub_total');
                    $isValid = round($trx->total, 2) == round($detailTotal, 2);
                @endphp

                @if ($isValid)
                    <span class="badge badge-success">Valid</span>
                @else
                    <span class="badge badge-error">Tidak Valid</span>
                @endif
            @endscope
            ...
            @scope('actions', $trx)
                ...
                <div class="flex gap-1">
                    @if (!$isValid)
                        <x-button icon="o-arrow-path" wire:click="syncTotal({{ $trx->id }})"
                            class="btn-ghost btn-sm text-blue-500" tooltip="Sinkronkan Total" />
                    @endif
                </div>
            @endscope
    ...
    <x-drawer wire:model="drawer" title="Filters" right separator with-close-button class="lg:w-1/3">
        <div class="grid gap-5">
            ...
            {{-- 🔥 FILTER VALID --}}
            <x-select label="Status Validasi" wire:model.live="validStatus" :options="[['id' => 'valid', 'name' => 'Valid'], ['id' => 'invalid', 'name' => 'Tidak Valid']]" placeholder="Semua" />
