...
new class extends Component {
    ...
    public function delete($id): void
    {
        ...
        $role->delete();
        $this->warning("Role $role->name akan dihapus", position: 'toast-top');
    }
    ...
    public function headers(): array
    {
        return [
            ...
            ['key' => 'nama', 'label' => 'Name', 'class' => 'w-64'], 
...
<div>
    ...
    <!-- TABLE wire:poll.5s="Roles"  -->
    <x-card>
        <x-table :headers="$headers" :rows="$roles" :sort-by="$sortBy" with-pagination
            link="roles/{id}/edit?name={name}&role={role.name}">
            @scope('actions', $role)
                <x-button icon="o-trash" wire:click="delete({{ $role['id'] }})"
                    wire:confirm="Yakin ingin menghapus {{ $role['name'] }}?" spinner
