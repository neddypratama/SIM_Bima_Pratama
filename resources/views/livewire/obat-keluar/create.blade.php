...
new class extends Component {
    ...
    public function mount(): void
    {
        ...
        $this->kategori_id = Kategori::where('name', 'like', '%Penjualan Obat%')->first()->id;
    ...
    public function updatedTanggal($value): void
    {
        if ($value) {
            ...
            $this->invoice2 = 'INV-' . $tanggal . '-OBT-' . $str;
            $this->invoice3 = 'INV-' . $tanggal . '-HPP-' . $str;
        }
    }

    private function hitungHppFifo(int $barangId, int $qty): float
    {
        $sisa = $qty;
        $totalHpp = 0;
    ...
    public function updatedDetails($value, $key): void
    {
        ...
        if (str_ends_with($key, '.kuantitas')) {
            ...
            if (!empty($this->details[$index]['barang_id'])) {
                $this->details[$index]['hpp'] = $this->hitungHppFifo($this->details[$index]['barang_id'], $qty);
            }
        }
    ...
    public function save(): void
    {
        ...
        $this->success('Transaksi berhasil dibuat!', redirectTo: '/obat-keluar');
    }
...
<div class="p-4 space-y-6">
    <x-header title="Create Transaksi Penjualan Obat" separator progress-indicator />
